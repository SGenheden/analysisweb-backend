...
import analysisweb.api.utils as utils
from analysisweb_user.models import (
    Measurement,
    Analysis,
    Job,
    JobInput,
...
class JobInputFile(Raw):
    def format(self, value):
        ...
        if os.path.exists(
            os.path.join(
                current_app.config["JOB_FILES_FOLDER"], str(value.job_id), "input", val
            )
        ):
...
class JobResource(ResourceBase):
    ...
    job_outputfile = {
        "label": String,
        "path": String(
            attribute=lambda x: "files/job/{}/output/{}".format(x.job.id, x.path)
        ),
    }
    ...
    fields = {
        ...
        "status": String,
        "log": String(
            attribute=lambda x: "files/job/{}/{}".format(x.id, x.log) if x.log else ""
        ),
        "analysis": IDField,
        "measurement": IDField,
    ...
    def delete(self, id_):
        ...
        try:
            return self.delete_resource(
                current_app.config["JOB_FILES_FOLDER"], resource
            )
...
class JobListResource(ResourceBase):
    ...
    def _add_job(self):
        ...
        job = Job(
            label=request.form["label"], measurement=measurement, analysis=analysis
        )
        db.session.add(job)
        db.session.flush()
        job_id = job.id
        ...
        for f in ["reports", "output", "input"]:
            file_folder = os.path.join(
                current_app.config["JOB_FILES_FOLDER"], str(job_id), f
            )
    ...
    def _initiate_job(job, analysis, measurement):
        ...
        with open(inp_file, "w") as f:
            json.dump(inp, f)
        syx_file = os.path.join(
            current_app.config["ANALYSIS_FILES_FOLDER"],
            str(analysis.id),
            analysis.syx_file,
        )
        post_url = current_app.config["SERVER_URL"] + "job/{}/log".format(job.id)
        utils.sympathy_job.delay(
            inp_file, syx_file, current_app.config["SYMPATHY_EXEC"], post_url
        )
                ...
                if job_input.value.startswith("$measurement"):
                    ...
                    for f in measurement.files:
                        if f.label == analysis_input.label:
                            value = os.path.join(
                                current_app.config["MEASUREMENT_FILES_FOLDER"],
                                str(measurement.id),
                                f.path,
                            )
                            break
                    ...
                    value = os.path.join(
                        current_app.config["JOB_FILES_FOLDER"],
                        str(job.id),
                        "input",
                        job_input.value,
                    )
            input_.append({"label": analysis_input.label, "value": value})
    ...
    def _validate_form_data():
        if (
            ...
            or "analysis" not in request.form
    ...
    def _validate_job_input(analysis, measurement):
        ...
        if ninput != nexpected:
            raise ResourceInvalidInputException(
                "Too few or too many input values, expecting {} but got {}".format(
                    nexpected, ninput
                )
            )
        ...
        for i, (analysis_input, item) in enumerate(zip(analysis.input, input_list)):
            ...
            if analysis_input.type.lower() is "value" and is_ref:
                raise ResourceInvalidInputException(
                    "Expected a 'value' input at position {} but found a reference".format(
                        i
                    )
                )
            if item.startswith("$measurement") and measurement is None:
                ...
                and analysis_input.label not in measurement_labels
            ):
                raise ResourceInvalidInputException(
                    "The analysis input label '{}' does not correspond to "
                    "any measurement label".format(analysis_input.label)
                )
...
class JobOutputResource(ResourceBase):
    ...
    def _add_output(self, resource):
        ...
        file_folder = os.path.join(
            current_app.config["JOB_FILES_FOLDER"], str(resource.id), "output"
        )
    ...
    def _add_figure(job, label, path):
        if label + ".fig" not in request.files or label + ".html" not in request.files:
            raise ResourceInvalidInputException(
                "Missing file with label {}".format(label)
            )
        ...
        if not filename_fig.lower().endswith(
            ".png"
        ) or not filename_html.lower().endswith(".html"):
            raise ResourceInvalidInputException(
...
class JobReportResource(ResourceBase):
    ...
    def _add_report(resource):
        ...
        file_folder = os.path.join(
            current_app.config["JOB_FILES_FOLDER"], str(resource.id), "reports"
        )
...
class JobLogResource(ResourceBase):
    ...
    def _add_job(resource):
        ...
        file_folder = os.path.join(
            current_app.config["JOB_FILES_FOLDER"], str(resource.id)
        )
