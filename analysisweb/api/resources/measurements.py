...
class MeasurementResource(ResourceBase):
    ...
    measurement_file = {
        "label": String,
        "path": String(attribute=lambda x: "files/measurement/{}/{}".format(x.measurement_id, x.path))
    }
    ...
    def delete(self, id):
        ...
        try:
            return self.delete_resource(current_app.config['MEASUREMENT_FILES_FOLDER'], resource)
    ...
    def _update_measurement(self, resource):
        start_date, end_date = self.parse_dates(str(resource.start_date), str(resource.end_date))
...
class MeasurementListResource(ResourceBase):
    ...
    def _add_measurement(self):
        ...
        m = Measurement(start_date=start_date, end_date=end_date, label=request.form['label'])
        db.session.add(m)
        db.session.flush()
        measurement_id = m.id
        self.load_metadata(request.form.get("meta_data", "{}"), m)
        file_folder = os.path.join(current_app.config['MEASUREMENT_FILES_FOLDER'], str(measurement_id))
    ...
    def _validate_form_data():
        if "start_date" not in request.form or "end_date" not in request.form \
                or "label" not in request.form or not request.files:
